 ...
 */
export const WEBRTC_CODE: number = protocols('webrtc').code
...
export interface WebRTCNoNatTransportInit {
  rtcConfiguration?: RTCConfiguration
}

export interface WebRTCNoNatTransportComponents {
  peerId: PeerId
  registrar: Registrar
  upgrader: Upgrader
...
export class WebRTCNoNatTransport implements Transport, Startable {
  ...
  constructor (
    private readonly components: WebRTCNoNatTransportComponents,
  ) {
  }
  ...
  createListener (options: CreateListenerOptions): Listener {
    ...
    // TODO vmx 2023-05-03: Think again whether having a single listener per transport still makes sense.
  ...
  async dial (ma: Multiaddr, options: DialOptions): Promise<Connection> {
    ...
    const result = await options.upgrader.upgradeOutbound(
      ...
      }),
      {
        skipProtection: true,
        skipEncryption: true,
        muxerFactory: new DataChannelMuxerFactory({ peerConnection: connection.connection })
      }
    )
...
/**
 * Determine if a given multiaddr contains a WebRTC Direct Code (280),
