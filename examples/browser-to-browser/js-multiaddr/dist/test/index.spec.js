...
describe('helpers', () => {
    ...
    describe('.nodeAddress', () => {
        ...
        it('throws on an invalid protocol name when the addr has an invalid one', () => {
            expect(() => multiaddr('/ip5/127.0.0.1/udp/5000')).to.throw(/no protocol with name/);
        });
        it('throws on an invalid protocol name when the transport protocol is not valid', () => {
            expect(() => multiaddr('/ip4/127.0.0.1/utp/5000')).to.throw(/no protocol with name/);
        });
    });
    ...
    describe('.getPeerId should parse id from multiaddr', () => {
        ...
        it('extracts the peer Id from a multiaddr, ipfs', () => {
            expect(multiaddr('/p2p-circuit/ipfs/QmcgpsyWgH8Y8ajJz1Cu72KnS5uo2Aa2LpzU7kinSupNKC').getPeerId()).to.equal('QmcgpsyWgH8Y8ajJz1Cu72KnS5uo2Aa2LpzU7kinSupNKC');
        });
